...
namespace RevitAPITraining_HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        ...
        {
            Document arDoc = commandData.Application.ActiveUIDocument.Document; // подключение док-ов АР 
            Document ovDoc = arDoc.Application.Documents.OfType<Document>().Where(x => x.Title.Contains("ОВ")).FirstOrDefault(); // связанный файл ОВ
            ...
            // надо убедится что в файл АР загружено семейство с отверстиями
            ...
            List<Duct> ducts = new FilteredElementCollector(ovDoc) // поиск всех воздуховодов
                .OfClass(typeof(Duct))
                .OfType<Duct>()
                .ToList();

            List<Pipe> pipes = new FilteredElementCollector(ovDoc) // поиск всех труб
               .OfClass(typeof(Pipe))
               .OfType<Pipe>()
               .ToList();

            View3D view3D = new FilteredElementCollector(arDoc) // найдем 3Д вид
            ...
            foreach (Duct duct in ducts)
            {
                ...
                XYZ point = curve.GetEndPoint(0);// точка
                XYZ direction = curve.Direction;// векор направление
                ...
                foreach (ReferenceWithContext refer in intersections)
                {
                    ...
                    FamilyInstance hole = arDoc.Create.NewFamilyInstance(pointHole, familySymbol, wall, level, StructuralType.NonStructural); // вставка объекта
